...
int projector(std::vector<int> orig3DxList, std::vector<int> orig3DyList, std::vector<int> orig3DzList, std::vector<int> point1List, std::vector<int> point2List, int x, int y, int z, int xAngDif, int yAngDif)
{
	...
	const float DFS = 30;
	std::vector<int> p1x(x3DList.size() + point1List.size(), 1000);//Default location for points is off the screen
	std::vector<int> p1y(x3DList.size() + point1List.size(), 1000);//so that points behind you arent rendered
	std::vector<int> p2x(x3DList.size() + point1List.size(), 1000);
	std::vector<int> p2y(x3DList.size() + point1List.size(), 1000);
	...
	{
		if(z3DList[point1List[n - x3DList.size()]] + z > 0 && z3DList[point2List[n - x3DList.size()]] + z > 0)//only moves points from off the screen (x = 1000 and y = 1000) if they are infront of the camera (after the rotation of the whole scene has been applied)
		{
		...
		if (z3DList[point1List[n - x3DList.size()]] + z < z3DList[point2List[n - x3DList.size()]] + z && z3DList[point2List[n - x3DList.size()]] + z > 0)
		{
			//std::cout << "p2z > p1z" << std::endl;
			int p1zv = z3DList[point1List[n - x3DList.size()]];
			int p2zv = z3DList[point2List[n - x3DList.size()]];
			int p1xv = x3DList[point1List[n - x3DList.size()]];
			int p2xv = x3DList[point2List[n - x3DList.size()]];
			int p1yv = y3DList[point1List[n - x3DList.size()]];
			int p2yv = y3DList[point2List[n - x3DList.size()]];
			int zDif = p2zv - p1zv;
			int xDif = ((p1xv > p2xv)? p1xv - p2xv : p2xv - p1xv);
			int yDif = ((p1yv > p2yv)? p1yv - p2yv : p2yv - p1yv);
			int cutX = ((p1xv > p2xv)? (int)((p2zv / zDif) * xDif) + p2xv : (int)((p2zv / zDif) * xDif) + p1xv);
			int cutY = ((p1yv > p2yv)? (int)((p2zv / zDif) * xDif) + p2yv : (int)((p2zv / zDif) * xDif) + p1yv);
			
			//x first
			p1x[n] = (int)(xRes / 2) + (int)((DFS / 1) * (cutX + x));
			p1y[n] = (int)(yRes / 2) + (int)((DFS / 1) * (cutY + y));//we can remove the "/ (z3DList[point2List[n - x3DList.size()]] + z" because this will allways be 1 as that is where we are 'cutting' the line to. (x / 1 = x)
			p2x[n] = (int)(xRes / 2) + (int)((DFS / (z3DList[point2List[n - x3DList.size()]] + z)) * (x3DList[point2List[n - x3DList.size()]] + x));
			p2y[n] = (int)(yRes / 2) + (int)((DFS / (z3DList[point2List[n - x3DList.size()]] + z)) * (y3DList[point2List[n - x3DList.size()]] + y));
			continue;
		}
		if (z3DList[point1List[n - x3DList.size()]] + z >= z3DList[point2List[n - x3DList.size()]] + z && z3DList[point1List[n - x3DList.size()]] + z > 0)
		{
			std::cout << "p1z > p2z" << n - x3DList.size() << std::endl;
			int p1zv = z3DList[point1List[n - x3DList.size()]];
			int p2zv = z3DList[point2List[n - x3DList.size()]];
			int p1xv = x3DList[point1List[n - x3DList.size()]];
			int p2xv = x3DList[point2List[n - x3DList.size()]];
			...
			int xDif = ((p1xv > p2xv)? p1xv - p2xv : p2xv - p1xv);
			int yDif = ((p1yv > p2yv)? p1yv - p2yv : p2yv - p1yv);
			int cutX = ((p1xv > p2xv)? (int)((p1zv / zDif) * xDif) + p2xv : (int)((p1zv / zDif) * xDif) + p1xv);
			int cutY = ((p1yv > p2yv)? (int)((p1zv / zDif) * xDif) + p2yv : (int)((p1zv / zDif) * xDif) + p1yv);
			
			//x first
			p1x[n] = (int)(xRes / 2) + (int)((DFS / 1) * (cutX + x));
			p1y[n] = (int)(yRes / 2) + (int)((DFS / 1) * (cutY + y));//we can remove the "/ (z3DList[point2List[n - x3DList.size()]] + z" because this will allways be 1 as that is where we are 'cutting' the line to. (x / 1 = x)
			p2x[n] = (int)(xRes / 2) + (int)((DFS / (z3DList[point1List[n - x3DList.size()]] + z)) * (x3DList[point1List[n - x3DList.size()]] + x));
			p2y[n] = (int)(yRes / 2) + (int)((DFS / (z3DList[point1List[n - x3DList.size()]] + z)) * (y3DList[point1List[n - x3DList.size()]] + y));
			continue;
		}
	}
