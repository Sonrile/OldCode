...
//Has a limit of (INPUT_SIZE) chars inputted
void get_string()
{
...
void deslang()
{
	...
	//If we don't find a x-x, we still copy the char from input[] to temp_str[]
	for (i = 0, t = 0; input[i] != '\0'; i++, t++) {
		if (input[i] == '-') {
			if ((is_letter(input[i-1]) == 1) && (is_letter(input[i+1]) == 1) && input[i-1] <= input[i+1]) {
				if (expand(input[i-1], input[i+1], t-1) != 1) {	/* send the chars to be expanded to the expand function */
					printf("Error: Output cannot be larger than %d chars", INPUT_SIZE - 1);	//If expand throws error, cancel expansion
					return;
				}
				t+=input[i+1] - input[i-1] - 1;		/* increase the temp string count by the amound of chars just expanded */
				need_repeat=1;
				print_char=0;
				i+=1;		/* Increase the input string's count by 1 so that the char "y" of "x-y" is skipped */
			}
			else if ((is_letter(input[i-1]) == 2) && (is_letter(input[i+1]) == 2) && input[i-1] <= input[i+1]) {
				if (expand(input[i-1], input[i+1], t-1) != 1) {
					printf("Error: Output cannot be larger than %d chars", INPUT_SIZE - 1);
					return;
				}
				t+=input[i+1] - input[i-1] - 1;
				need_repeat=1;
				print_char=0;
				i+=1;
			}
			else if (is_number(input[i-1]) && is_number(input[i+1]) && input[i-1] <= input[i+1]) {
				if (expand(input[i-1], input[i+1], t-1) != 1) {
					printf("Error: Output cannot be larger than %d chars", INPUT_SIZE - 1);
					return;
				}
